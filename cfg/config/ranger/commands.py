...
class zsh(Command):
    ...
    def execute(self):
        import subprocess
        subprocess.Popen(['/home/paul/.config/ranger/ranger_zsh', self.fm.thisdir.path, self.arg(1)])
...
class vim(Command):
    ...
    def execute(self):
        ...
        if not os.path.isdir(selection[0]):
            selection.insert(0, '/home/paul/.config/ranger/ranger_vi')
            selection.insert(1, self.arg(1))
        else:
            selection[0] = '/home/paul/.config/ranger/ranger_vi'
            selection.insert(1, self.arg(1))
...
class peco_open_popup(Command):
    ...
    def execute(self):
        ...
        p1 = subprocess.Popen(['/usr/bin/urxvt', '-name', 'one_command_shell_big', '-e', '/home/paul/.config/ranger/peco_popup'], stdout=FNULL, stderr=FNULL)
        p1.wait()
...
class fzf_select(Command):
    ...
    def execute(self):
        ...
        if self.quantifier:
            ...
            -o -type d -print 2> /dev/null | sed 1d | cut -b3- | ~/.fzf/bin/fzf +m"
        else:
            # match files and directories
            command="find -L . \( -path '*/\.*' -o -fstype 'dev' -o -fstype 'proc' \) -prune \
            -o -print 2> /dev/null | sed 1d | cut -b3- | ~/.fzf/bin/fzf +m"
...
class fzf_open(Command):
    ...
    def execute(self):
        import subprocess
        import os.path
        command="cat ~/.cache/ctrlp/mru/cache.txt | ~/.fzf/bin/fzf +m"
        fzf = self.fm.execute_command(command, universal_newlines=True, stdout=subprocess.PIPE)
        stdout, stderr = fzf.communicate()
        if fzf.returncode == 0:
            fzf_file = [os.path.abspath(stdout.rstrip('\n'))]
            ...
            if not os.path.isdir(fzf_file[0]):
                fzf_file.insert(0, '/home/paul/.config/ranger/ranger_vi')
            else:
                fzf_file[0] = '/home/paul/.config/ranger/ranger_vi'
